...
public class AssetsUtils {
                ...
                while (classifierNames.hasNext()) {
                    ...
                    if (libNode.has("natives")) {
                        ...
                        if (natives.has(classifier)) {
                            // This classifier has specific OS requirements
                            // For simplicity, we'll include all classifiers
    ...
    private static void downloadArtifact(DownloadTask task) throws IOException {
        ...
        System.out.println("Downloading " + task.name + "...");
        try {
            NetUtils.downloadArtifact(task.url, task.outputPath);
        } catch (Exception e) {
            e.printStackTrace();
        }
        System.out.println("Downloaded: " + task.outputPath);
    }
