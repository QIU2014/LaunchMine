...
import com.eric.ui.DownloadDialog;
import com.eric.ui.MinecraftLogWindow;
...
public class LaunchUtils {
    ...
    public static Process launchMinecraft(String versionId, String playerName,
        ...
        System.out.println("=== STARTING MINECRAFT LAUNCH ===");
        ...
        // Log version details
        System.out.println("Main Class: " + versionJson.get("mainClass").asText());
        System.out.println("Assets: " + versionJson.get("assets").asText());
        System.out.println("Type: " + versionJson.get("type").asText());
        ...
        System.out.println("Classpath length: " + classpath.length() + " chars");
        System.out.println("Classpath (first 500 chars): " + classpath.substring(0, Math.min(500, classpath.length())) + "...");
        ...
        if (started && process.exitValue() != 0) {
            int exitCode = process.exitValue();
            System.err.println("Process exited immediately with code: " + exitCode);
        ...
        System.out.println("Minecraft process started successfully");
    ...
    private static void validateAndDownloadFiles(JsonNode versionJson, String versionId) throws Exception {
        ...
        System.out.println("All files validated and ready!");
    ...
    private static void downloadAssets(String assetIndexPath) throws Exception {
        ...
        System.out.println("Checking " + totalAssets + " assets...");
        ...
        for (String key : (Iterable<String>) () -> objects.fieldNames()) {
            ...
            if (!isFileValid(filePath, hash, size)) {
                System.out.println("Downloading asset: " + key + " (" + hash.substring(0, 8) + ")");
                ...
                try {
                    ...
                    // Verify download
                    if (!isFileValid(filePath, hash, size)) {
                        System.err.println("Warning: Downloaded asset failed validation: " + key);
                        assetFile.delete(); // Delete corrupted file
                    }
                } catch (Exception e) {
                    System.err.println("Failed to download asset " + key + ": " + e.getMessage());
                }
            } else {
                skipped++;
            }
        ...
        if (downloaded > 0) {
            System.out.println("Downloaded " + downloaded + " new assets");
        }
        if (skipped > 0) {
            System.out.println("Skipped " + skipped + " existing assets");
        }
    }
    ...
    private static void validateOrDownloadFile(String url, String filePath,
        ...
        System.out.println("✓ " + description + " downloaded successfully");
                ...
                if (natives.has(osKey) && library.has("downloads") &&
                    ...
                    if (classifiers.has(nativeClassifier)) {
                        ...
                        if (nativeFile.exists()) {
                            System.out.println("Extracting native: " + nativeFile.getName());
    ...
    private static boolean isFileValid(String filePath, String expectedSha1, long expectedSize) {
        ...
        if (actualSize != expectedSize) {
            System.out.println("File size mismatch: " + filePath +
        ...
        if (expectedSize > 100 * 1024 * 1024) { // > 100MB
            System.out.println("Skipping SHA1 check for large file: " + filePath);
        ...
        try {
            ...
            if (!actualSha1.equalsIgnoreCase(expectedSha1)) {
                System.out.println("SHA1 mismatch for: " + filePath +
                        " (expected: " + expectedSha1 + ", got: " + actualSha1 + ")");
                return false;
            }
            return true;
        } catch (Exception e) {
            System.err.println("Error calculating SHA1 for " + filePath + ": " + e.getMessage());
            return false;
        }
    }
    ...
    private static String buildClasspath(JsonNode versionJson, String versionId) {
        ...
        if (classpath.toString().split(File.pathSeparator).length < 10) {
            System.out.println("Warning: Few libraries found in version.json, scanning libraries directory...");
    ...
    private static void addFallbackLibraries(StringBuilder classpath) {
        ...
        System.out.println("Added " + addedCount + " libraries from fallback scan");
    }
    ...
    public static void validateVersionOnStartup(String versionId) {
        try {
            ...
            if (!versionJsonFile.exists()) {
                System.out.println("Version " + versionId + " not found, skipping validation");
                return;
            }
    ...
    public static void validateAllVersionsOnStartup() {
        ...
        System.out.println("Validating all installed versions...");
        ...
        for (File versionFolder : versionFolders) {
            ...
            if (versionJson.exists()) {
                System.out.println("Validating: " + versionId);
                validateVersionOnStartup(versionId);
            }
        }

        System.out.println("Version validation complete!");
    }
}